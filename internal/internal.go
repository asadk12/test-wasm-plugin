...
func (r *RequestHandler) OnHttpRequestHeaders(numHeaders int, endOfStream bool) types.Action {
	proxywasm.LogWarnf("WASM !request")
	...
	if err := proxywasm.AddHttpRequestHeader("X-test-wasm-header", "injected-from-wasm-plugin"); err != nil {
		proxywasm.LogCriticalf("failed to inject custom header: %v", err)
	}
